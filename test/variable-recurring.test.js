...
describe('Variable Recurring', () => {
  ...
  before(function () {
    ...
    this.variableRecurring = new VariableRecurring(this.web3, this.contractLoader);
  });

  context('bill', function () {
    it('should bill subscriptions', function () {
      const contract = this.contractLoader.load('VariableRecurringSubscriptionsManagement');
      const method = contract.methods.bill(planId, subscriptionIds, amounts);
      const transaction = this.variableRecurring.bill(planId, subscriptionIds, amounts);
    ...
    it('should throw an error if planId in invalid', function () {
      ...
      const fn = () => this.variableRecurring.bill(invalidPlanId, subscriptionIds, amounts);
    ...
    it('should throw an error if subscriptionIds in not an array', function () {
      ...
      const fn = () => this.variableRecurring.bill(planId, {}, amounts);
    ...
    it('should throw an error if amounts in not an array', function () {
      ...
      const fn = () => this.variableRecurring.bill(planId, subscriptionIds, {});
    ...
    it('should throw an error if a subscriptionId in invalid', function () {
      ...
      const fn = () => this.variableRecurring.bill(planId, [invalidSubscriptionId], amounts);
    ...
    it('should throw an error if an amount in invalid', function () {
      ...
      const fn = () => this.variableRecurring.bill(planId, subscriptionIds, [invalidAmount]);
  ...
  context('terminate', function () {
    it('should terminate subscriptions', function () {
      ...
      const transaction = this.variableRecurring.terminate(planId, subscriptionIds);
    ...
    it('should throw an error if planId in invalid', function () {
      const invalidPlanId = 'my-plan-id';
      const error = new InvalidPlanIdError(invalidPlanId);
      const fn = () => this.variableRecurring.terminate(invalidPlanId, subscriptionIds);
    ...
    it('should throw an error if subscriptionIds in not an array', function () {
      const error = new NotAnArrayError('subscriptionIds');
      const fn = () => this.variableRecurring.terminate(planId, {});
    ...
    it('should throw an error if a subscriptionId in invalid', function () {
      const invalidSubscriptionId = 'invalid-subscription-id';
      const error = new InvalidSubscriptionIdError(invalidSubscriptionId);
      const fn = () => this.variableRecurring.terminate(planId, [invalidSubscriptionId]);
